...
use memhack::{MemHook, MemHackError};
...
impl Savestate {
    ...
    fn write_value(&self, memhook: &MemHook) -> Result<(), MemHackError> {
        memhook.write_val_ptr(self.base, &self.offsets[0], self.savestates[self.index - 1].unwrap().0)?;
        memhook.write_val_ptr(self.base, &self.offsets[1], self.savestates[self.index - 1].unwrap().1)?;
        memhook.write_val_ptr(self.base, &self.offsets[2], self.savestates[self.index - 1].unwrap().2)?;
        Ok(())
    }
}
...
impl Widget for Savestate {
    fn draw(&mut self, ui: &mut egui::Ui, memhook: &memhack::MemHook) {
        ui.horizontal(|ui| {
            ...
            let response = ui.add_enabled(self.savestates[self.index - 1].is_some(), Button::new("Load"));
            if response.clicked() {
