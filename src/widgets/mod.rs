...
const FALL_DAMAGE_BYTES: &[u8] = &[0x55, 0x48, 0x8B, 0xEC, 0x57, 0x48, 0x83, 0xEC, 0x08, 0x48, 0x8B, 0xF9, 0x48, 0x89, 0x55, 0xF0, 0x0F, 0xB6, 0x47, 0x28, 0x85, 0xC0, 0x0F, 0x84, 0x3F, 0x00, 0x00, 0x00, 0x48, 0x8B, 0xCF, 0x48, 0x83, 0xEC, 0x20];
...
impl Widgets {
    pub fn new() -> Self {
        ...
        let fall_damage_hack = Box::new(CheckboxAOB::new("Disable Fall Damage".to_string(), FALL_DAMAGE_BYTES.to_vec(), (Some(0x2000000), None), vec![0xC3], vec![0x55]));

        Self {
            widgets: vec![x_pos_edit, y_pos_edit, z_pos_edit, fall_damage_hack],
            memhook,
        }
    }
